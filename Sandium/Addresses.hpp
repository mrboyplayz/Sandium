...
namespace addresses
{
...
#if _WIN32
    using DrawHUDFuncType = void (std::int64_t, std::int64_t, int, int);
    extern FuncAddress<DrawHUDFuncType> DrawHUDFunc;
#endif
    ...
    // String text entry ("Server Name"/"Password" style): (label, char buffer, max length)
    using DrawMenuTextEntryFuncType = std::int64_t (const char *text, char *buffer, unsigned int maxLength);
    extern FuncAddress<DrawMenuTextEntryFuncType> DrawMenuTextEntryFunc;
