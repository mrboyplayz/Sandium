// Compact SHA-256 (FIPS 180-4), shared by the Sandium DLL and the launcher.
// Exposes sha256_hex(text) -> 64-char lowercase hex.
#pragma once
#include <stdint.h>
#include <string.h>
...
static void sha256_final(sha256_ctx *ctx, uint8_t hash[32])
{
    ...
    for (int j = 0; j < 4; ++j)
        for (int k = 0; k < 8; ++k)
            hash[j * 8 + k] = (uint8_t)(ctx->state[k] >> (24 - j * 8));
}
