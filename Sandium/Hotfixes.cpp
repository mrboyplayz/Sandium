...
bool hotfixes::Make(std::uintptr_t baseAddress)
{
    ...
    // Force Steam auth state so client proceeds to master server
    if (!*addresses::IsDedicated.ptr)
    {
        if (addresses::SteamEnabled.ptr)
        {
            addresses::SteamEnabled.ptr->b1 = true;
            addresses::SteamEnabled.ptr->b2 = true;
            addresses::SteamEnabled.ptr->b3 = true;
            addresses::SteamEnabled.ptr->b4 = true;
        }
        if (addresses::SteamTicketRetrieved.ptr)
        {
            addresses::SteamTicketRetrieved.ptr->b1 = true;
            addresses::SteamTicketRetrieved.ptr->b2 = true;
            addresses::SteamTicketRetrieved.ptr->b3 = true;
            addresses::SteamTicketRetrieved.ptr->b4 = true;
        }
        if (addresses::SteamTicketLength.ptr)
            *addresses::SteamTicketLength.ptr = 256; // dummy ticket length
        if (addresses::SteamTicketBuffer.ptr)
        {
            // Write a dummy Steam ticket
            std::memset(addresses::SteamTicketBuffer.ptr, 0xAA, 256);
        }
        if (addresses::AuthStatus.ptr)
        {
            *addresses::AuthStatus.ptr = 1; // authenticated
        }
    }
