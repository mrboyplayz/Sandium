...
local function newHit(bone, bBody, aBody)
    ...
    -- Swinging limbs can collide with the map, props, or stale rigid bodies.
    -- Only human-on-human contacts are valid boxing hits.
    if not VicMan or not AttackMan or not AttackMan.data.BoxingData then
        return
    end
...
plugin:addHook("Logic", function()
    for _, man in ipairs(humans.getAll()) do
        ...
        -- Plugin hot reloads keep existing humans, so PostHumanCreate is not
        -- called again. Repair their boxing state and rigid-body ownership.
        if not man.data.BoxingData then
            man.data.BoxingData = {
                KBMulti = Config.KnockbackMultiplier,
                MaxDMG = man.data.isPunching and 95 or 1000,
            }

            for i = 0, 15 do
                local body = man:getRigidBody(i)
                body.data.RBHuman = man
                body.data.bone = i
            end
        end
        ...
        if bit32.band(man.inputFlags, 16) == 16 and bit32.band(man.inputFlags, 8) == 8 and bit32.band(man.inputFlags, 1) == 1 and not man.data.PunchDebounce then
...
plugin:addHook("CollideBodies", function(aBody, bBody, aLocalPos, bLocalPos)
    if (bBody.data.SwingLeft or bBody.data.SwingRight) and not bBody.data.HasHit then
        local a = blocklogic(aBody, bBody)
        local b = blocklogic(bBody, aBody)

        if not a and not b then
            newHit(aBody.data.bone,bBody,aBody)
        end

    elseif (aBody.data.SwingLeft or aBody.data.SwingRight) and not aBody.data.HasHit then
        local a = blocklogic(bBody, aBody)
        local b = blocklogic(aBody, bBody)
